...
- `input_modalities` combines explicit marker declarations with reported Ollama capabilities. `completion` adds `text`; `vision` adds `image`. The router does not assume image support for every completion model.
